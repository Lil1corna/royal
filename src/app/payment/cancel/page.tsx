...
export default async function PaymentCancelPage(props: {
  ...
  let order: { payment_status: string } | null = null
  ...
  return (
    <main className="mt-20 max-w-xl mx-auto p-4 md:p-6 lg:p-8">
      <div className="ds-card-glass rounded-2xl p-6 md:p-8 text-center">
        ...
        <h1 className="text-3xl font-bold mb-3 text-white">Оплата отменена</h1>
        <p className="text-neutral-300 mb-2">Номер заказа: {orderId ? `#${orderId.slice(0, 8)}` : 'не указан'}</p>
        <p className="text-neutral-400 mb-6">
          Статус оплаты: <span className="text-neutral-200 font-semibold">{order?.payment_status || 'pending'}</span>
        </p>
        <div className="flex flex-wrap items-center justify-center gap-3">
          <Link href="/cart" className="btn-primary px-6 py-3">
            Вернуться в корзину
          </Link>
          <Link href="/" className="ds-btn-secondary px-6 py-3">
            На главную
          </Link>
        </div>
      </div>
